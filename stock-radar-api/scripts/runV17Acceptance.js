...
async function main() {
  ...
  const logFile = path.join(logDir, `v17-acceptance-${formatDateTime(startedAt)}.log`);
  ...
  appendLine(logStream, "====================================");
  appendLine(logStream, "Stock Radar V1.7 自動驗收測試");
  appendLine(logStream, "====================================");
  ...
  if (!skipDb) {
    tasks.push({ title: "建立 V1.7 技術突破資料表", command: "npm", args: ["run", "breakout:setup"], cwd: apiDir });
    tasks.push({ title: "產生技術突破訊號", command: "npm", args: ["run", "breakout:generate", ...(tradeDate ? ["--", tradeDate] : [])], cwd: apiDir });
  }

  tasks.push({ title: "V1.7 靜態 / API 驗收", command: "npm", args: ["run", "v17:check", ...(apiBaseUrl ? ["--", `--api=${apiBaseUrl}`] : [])], cwd: apiDir });
...
main().catch((error) => {
  console.error("V1.7 自動驗收測試失敗：", error);
  process.exit(1);
});
