...
import { importInstitutionalAmountSummariesForDate } from "./importInstitutionalAmountSummaries.js";
...
function parseArgs(argv) {
  ...
  if (!options.startDate || !options.endDate) {
    throw new Error("請提供起訖日期，例如：npm run official:institutional-amounts:history -- 2026-01-01 2026-06-18");
  }
...
async function getExistingInstitutionalCount(conn, tradeDate) {
...
async function main() {
  ...
  try {
    ...
    for (const item of eachDate(options.startDate, options.endDate)) {
      ...
      const existingCount = await getExistingInstitutionalCount(conn, item.dateText);
      ...
      try {
        const result = await importInstitutionalAmountSummariesForDate(conn, item.dateText);
        const marketList = result.results.map((row) => row.market_type).join("、");
    ...
    console.log("三大法人官方金額歷史回補完成");
    console.log(JSON.stringify(summary, null, 2));
  } catch (error) {
    summary.failed += 1;
    console.error("三大法人官方金額歷史回補失敗");
