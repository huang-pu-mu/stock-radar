...
async function main() {
  ...
  const logFile = path.join(logDir, `v16-acceptance-${formatDateTime(startedAt)}.log`);
  ...
  appendLine(logStream, "====================================");
  appendLine(logStream, "Stock Radar V1.6 自動驗收測試");
  appendLine(logStream, "====================================");
  ...
  appendLine(logStream, `DB流程：${skipDb ? "略過" : "執行 setup / import / score"}`);
  ...
  if (!skipDb) {
    tasks.push({ title: "建立 V1.6 全球市場風險資料表", command: "npm", args: ["run", "global-risk:setup"], cwd: apiDir });
    if (!skipImport) tasks.push({ title: "匯入全球市場風險資料", command: "npm", args: ["run", "global-risk:import", ...(tradeDate ? ["--", `--date=${tradeDate}`] : [])], cwd: apiDir });
    tasks.push({ title: "產生全球風險修正分數", command: "npm", args: ["run", "global-risk:score", ...(tradeDate ? ["--", tradeDate] : [])], cwd: apiDir });
  }

  tasks.push({ title: "V1.6 靜態 / API 驗收", command: "npm", args: ["run", "v16:check", ...(apiBaseUrl ? ["--", `--api=${apiBaseUrl}`] : [])], cwd: apiDir });
...
main().catch((error) => {
  console.error("V1.6 自動驗收測試失敗：", error);
  process.exit(1);
});
