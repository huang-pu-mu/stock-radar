...
async function main() {
  ...
  const logPath = path.join(logDir, `v25-acceptance-${formatDateTime(startedAt)}.log`);
  ...
  appendLine(logStream, "====================================");
  appendLine(logStream, "Stock Radar V2.5 每日投資作戰室自動驗收測試");
  appendLine(logStream, "====================================");
  ...
  appendLine(logStream, skipDb ? "DB流程：略過 setup / war room generate" : "DB流程：執行 setup / war room generate");
  ...
  const tasks = [
    ...
    { title: "V2.5 setup script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "setupDailyWarRoomTables.js")], cwd: apiDir },
    { title: "V2.5 generate script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "generateDailyWarRoomReport.js")], cwd: apiDir },
  ];

  if (!skipDb) {
    tasks.push(
      { title: "建立 V2.5 每日投資作戰室資料表", command: "npm", args: ["run", "war-room:setup"], cwd: apiDir },
      { title: "產生 V2.5 每日投資作戰室報告", command: "npm", args: ["run", "war-room:generate"], cwd: apiDir },
    );
  }

  const checkArgs = ["run", "v25:check"];
  if (apiBaseUrl) checkArgs.push("--", `--api=${apiBaseUrl}`);
  tasks.push({ title: "V2.5 靜態 / API 驗收", command: "npm", args: checkArgs, cwd: apiDir });
...
main().catch((error) => {
  console.error("V2.5 自動驗收測試失敗：", error);
  process.exit(1);
});
