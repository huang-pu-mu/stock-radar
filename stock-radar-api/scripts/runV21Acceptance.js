...
async function main() {
  ...
  const logFile = path.join(logDir, `v21-acceptance-${formatDateTime(startedAt)}.log`);
  ...
  appendLine(logStream, "====================================");
  appendLine(logStream, "Stock Radar V2.1 持股與風控管理自動驗收測試");
  appendLine(logStream, "====================================");
  ...
  appendLine(logStream, `DB流程：${skipDb ? "略過" : "執行 setup / snapshot / alerts"}`);
  ...
  const tasks = [
    ...
    { title: "V2.1 setup script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "setupPositionRiskTables.js")], cwd: apiDir },
    { title: "V2.1 snapshot script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "generatePositionSnapshots.js")], cwd: apiDir },
    { title: "V2.1 alerts script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "generatePositionRiskAlerts.js")], cwd: apiDir },
  ];

  if (!skipDb) {
    tasks.push({ title: "建立 V2.1 持股與風控資料表", command: "npm", args: ["run", "position:setup"], cwd: apiDir });
    tasks.push({ title: "產生 V2.1 持股風險快照", command: "npm", args: ["run", "position:snapshot", ...(tradeDate ? ["--", tradeDate] : [])], cwd: apiDir });
    tasks.push({ title: "產生 V2.1 持股風控提醒", command: "npm", args: ["run", "position:alerts", ...(tradeDate ? ["--", tradeDate] : [])], cwd: apiDir });
  }

  tasks.push({ title: "V2.1 靜態 / API 驗收", command: "npm", args: ["run", "v21:check", ...(apiBaseUrl ? ["--", `--api=${apiBaseUrl}`] : [])], cwd: apiDir });
...
main().catch((error) => {
  console.error("V2.1 自動驗收測試失敗：", error);
  process.exit(1);
});
