...
async function main() {
  ...
  const sqlSource = readText("stock-radar-api/sql/v31-pre-trade-preparation.sql");
  ...
  checks.push(createCheck("版本", "API 版本為 V3.1.0", serverSource.includes('stock-radar-api-v3.1.0'), "stock-radar-api-v3.1.0"));
  checks.push(createCheck("版本", "API 預期 PWA 版本為 v78", serverSource.includes('stock-radar-pwa-v78'), "stock-radar-pwa-v78"));
  checks.push(createCheck("版本", "service-worker 快取版本為 v78", swSource.includes('stock-radar-pwa-v78'), "stock-radar-pwa-v78"));
  checks.push(createCheck("前端", "側邊功能選單顯示 V3.1", indexSource.includes("V3.1 半自動交易前置準備"), "index.html"));

  const scripts = [
    ["pre-trade:setup", "node scripts/setupPreTradePreparationTables.js"],
    ["pre-trade:generate", "node scripts/generatePreTradePreparations.js"],
    ["pre-trade:daily", "npm run pre-trade:setup && npm run pre-trade:generate"],
    ["v31:check", "node scripts/checkV31.js"],
  ...
  const requiredFiles = [
    "stock-radar-api/sql/v31-pre-trade-preparation.sql",
    "stock-radar-api/scripts/setupPreTradePreparationTables.js",
    "stock-radar-api/scripts/generatePreTradePreparations.js",
    "stock-radar-api/scripts/checkV31.js",
    "stock-radar-api/scripts/runV31Acceptance.js",
  ...
  const sqlMarkers = [
    ["交易前計畫資料表", "CREATE TABLE IF NOT EXISTS `pre_trade_plans`"],
    ["交易前檢查項目資料表", "CREATE TABLE IF NOT EXISTS `pre_trade_check_items`"],
    ["操作紀錄資料表", "CREATE TABLE IF NOT EXISTS `pre_trade_action_logs`"],
  ...
  const frontendMarkers = [
    ["交易前準備頁", "renderPreTradePage"],
    ["交易前摘要 API", "/pre-trade/summary"],
    ["交易前檢查清單 API", "/pre-trade/checklists"],
    ["產生前置檢查 API", "/pre-trade/generate"],
    ["V3.1 文字", "V3.1 半自動交易前置準備"],
    ["交易前準備按鈕", "data-pre-trade-generate"],
  ];
  ...
  checks.push(createCheck("前端", "不再顯示 V1.4 系統狀態", !appSource.includes("V1.4 系統狀態") && !appSource.includes("V1.4 功能完成度") && !appSource.includes("重新檢查 V1.4"), "V3.1 status copy"));
  checks.push(createCheck("安全邊界", "V3.1 不串券商、不自動下單", serverSource.includes("不串券商") && serverSource.includes("不會自動下單"), "manual confirmation only"));
  ...
  if (apiBaseUrl) {
    ...
    const apiChecks = [
      ...
      ["/v31/status", `${apiBaseUrl}/v31/status`, 200],
      ["/v31/acceptance", `${apiBaseUrl}/v31/acceptance`, 200],
      ["/pre-trade/summary", `${apiBaseUrl}/pre-trade/summary`, 200],
      ["/pre-trade/checklists", `${apiBaseUrl}/pre-trade/checklists`, 200],
      ["/pre-trade/plans 登入保護", `${apiBaseUrl}/pre-trade/plans`, 401],
    ];
  ...
  console.log("====================================");
  console.log("Stock Radar V3.1 半自動交易前置準備驗收檢查");
  console.log("====================================");
...
main().catch((error) => {
  console.error("V3.1 驗收檢查失敗：", error);
  process.exit(1);
});
