...
async function main() {
  ...
  const snapshotDate = args.find((arg) => /^\d{4}-\d{2}-\d{2}$/.test(arg)) || "";
  const logPath = path.join(logDir, `v24-acceptance-${formatDateTime(startedAt)}.log`);
  const logStream = fs.createWriteStream(logPath, { flags: "a" });
  const failures = [];
  const warnings = [];

  appendLine(logStream, "====================================");
  appendLine(logStream, "Stock Radar V2.4 部位模擬與風險觀察自動驗收測試");
  appendLine(logStream, "====================================");
  ...
  appendLine(logStream, `DB流程：${skipDb ? "略過" : "執行 setup / portfolio risk"}`);
  ...
  const tasks = [
    ...
    { title: "V2.4 setup script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "setupPortfolioRiskTables.js")], cwd: apiDir },
    { title: "V2.4 risk script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "generatePortfolioRiskSnapshots.js")], cwd: apiDir },
  ];

  if (!skipDb) {
    tasks.push({ title: "建立 V2.4 部位模擬與風險觀察資料表", command: "npm", args: ["run", "portfolio:setup"], cwd: apiDir });
    tasks.push({ title: "產生 V2.4 部位風險快照", command: "npm", args: ["run", "portfolio:risk", ...(snapshotDate ? ["--", snapshotDate] : [])], cwd: apiDir });
  }

  tasks.push({ title: "V2.4 靜態 / API 驗收", command: "npm", args: ["run", "v24:check", ...(apiBaseUrl ? ["--", `--api=${apiBaseUrl}`] : [])], cwd: apiDir });
...
main().catch((error) => {
  console.error("V2.4 自動驗收測試失敗：", error);
  process.exit(1);
});
