...
async function main() {
  ...
  const logFile = path.join(logDir, `v22-acceptance-${formatDateTime(startedAt)}.log`);
  ...
  appendLine(logStream, "====================================");
  appendLine(logStream, "Stock Radar V2.2 交易紀錄與績效分析自動驗收測試");
  appendLine(logStream, "====================================");
  ...
  const tasks = [
    ...
    { title: "V2.2 setup script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "setupTradePerformanceTables.js")], cwd: apiDir },
    { title: "V2.2 performance script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "generateTradePerformance.js")], cwd: apiDir },
  ];

  if (!skipDb) {
    tasks.push({ title: "建立 V2.2 交易紀錄與績效資料表", command: "npm", args: ["run", "trade:setup"], cwd: apiDir });
    tasks.push({ title: "產生 V2.2 交易績效快照", command: "npm", args: ["run", "trade:performance", ...(tradeDate ? ["--", tradeDate] : [])], cwd: apiDir });
  }

  tasks.push({ title: "V2.2 靜態 / API 驗收", command: "npm", args: ["run", "v22:check", ...(apiBaseUrl ? ["--", `--api=${apiBaseUrl}`] : [])], cwd: apiDir });
...
main().catch((error) => {
  console.error("V2.2 自動驗收測試失敗：", error);
  process.exit(1);
});
