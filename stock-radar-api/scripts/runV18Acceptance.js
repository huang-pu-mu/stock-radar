...
async function main() {
  ...
  const logFile = path.join(logDir, `v18-acceptance-${formatDateTime(startedAt)}.log`);
  ...
  appendLine(logStream, "====================================");
  appendLine(logStream, "Stock Radar V1.8 自動驗收測試");
  appendLine(logStream, "====================================");
  ...
  if (!skipDb) {
    tasks.push({ title: "建立 V1.8 主力籌碼資料表", command: "npm", args: ["run", "main-force:setup"], cwd: apiDir });
    tasks.push({ title: "產生主力籌碼訊號", command: "npm", args: ["run", "main-force:generate", ...(tradeDate ? ["--", tradeDate] : [])], cwd: apiDir });
  }

  tasks.push({ title: "V1.8 靜態 / API 驗收", command: "npm", args: ["run", "v18:check", ...(apiBaseUrl ? ["--", `--api=${apiBaseUrl}`] : [])], cwd: apiDir });
...
main().catch((error) => {
  console.error("V1.8 自動驗收測試失敗：", error);
  process.exit(1);
});
