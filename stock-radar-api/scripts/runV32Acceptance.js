  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      if (code === 0) {
        ...
        appendLine(logStream, `警告：${title}，exit code ${code}，耗時 ${duration}`);
        resolve({ code, warning: `${title} exit code ${code}` });
      } else {
        reject(new Error(`${title} 失敗，exit code ${code}`));
      }
    });
...
async function main() {
  ...
  const logPath = path.join(logDir, `v32-acceptance-${formatDateTime(startedAt)}.log`);
  ...
  appendLine(logStream, "====================================");
  appendLine(logStream, "Stock Radar V3.2 AI 每日推薦引擎自動驗收測試");
  appendLine(logStream, "====================================");
  appendLine(logStream, `工作目錄：${apiDir}`);
  appendLine(logStream, `Log 檔案：${logPath}`);
  appendLine(logStream, `開始時間：${startedAt.toLocaleString("zh-TW", { hour12: false })}`);
  appendLine(logStream, apiBaseUrl ? `API：${apiBaseUrl}` : "API：未指定，只做靜態檢查");
  appendLine(logStream, skipDb ? "DB流程：略過 setup / ai-recommendations generate" : "DB流程：執行 setup / ai-recommendations generate");
  ...
  const tasks = [
    ...
    { title: "V3.2 setup script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "setupAiDailyRecommendationsTables.js")], cwd: apiDir },
    { title: "V3.2 generate script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "generateAiDailyRecommendations.js")], cwd: apiDir },
  ];

  if (!skipDb) {
    tasks.push(
      { title: "建立 V3.2 AI 每日推薦引擎資料表", command: "npm", args: ["run", "ai-recommendations:setup"], cwd: apiDir },
      { title: "產生 V3.2 AI 每日推薦引擎清單", command: "npm", args: ["run", "ai-recommendations:generate"], cwd: apiDir },
    );
  }

  const checkArgs = ["run", "v32:check"];
  if (apiBaseUrl) checkArgs.push("--", `--api=${apiBaseUrl}`);
  tasks.push({ title: "V3.2 靜態 / API 驗收", command: "npm", args: checkArgs, cwd: apiDir });
...
main().catch((error) => {
  console.error("V3.2 自動驗收測試失敗：", error);
  process.exit(1);
});
