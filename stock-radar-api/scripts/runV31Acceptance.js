...
async function main() {
  ...
  const logPath = path.join(logDir, `v31-acceptance-${formatDateTime(startedAt)}.log`);
  ...
  appendLine(logStream, "====================================");
  appendLine(logStream, "Stock Radar V3.1 半自動交易前置準備自動驗收測試");
  appendLine(logStream, "====================================");
  ...
  appendLine(logStream, skipDb ? "DB流程：略過 setup / pre-trade generate" : "DB流程：執行 setup / pre-trade generate");
  ...
  const tasks = [
    ...
    { title: "V3.1 setup script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "setupPreTradePreparationTables.js")], cwd: apiDir },
    { title: "V3.1 generate script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "generatePreTradePreparations.js")], cwd: apiDir },
  ];

  if (!skipDb) {
    tasks.push(
      { title: "建立 V3.1 半自動交易前置準備資料表", command: "npm", args: ["run", "pre-trade:setup"], cwd: apiDir },
      { title: "產生 V3.1 半自動交易前置準備清單", command: "npm", args: ["run", "pre-trade:generate"], cwd: apiDir },
    );
  }

  const checkArgs = ["run", "v31:check"];
  if (apiBaseUrl) checkArgs.push("--", `--api=${apiBaseUrl}`);
  tasks.push({ title: "V3.1 靜態 / API 驗收", command: "npm", args: checkArgs, cwd: apiDir });
...
main().catch((error) => {
  console.error("V3.1 自動驗收測試失敗：", error);
  process.exit(1);
});
