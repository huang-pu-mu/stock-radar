...
async function main() {
  ...
  const logFile = path.join(logDir, `v23-acceptance-${formatDateTime(startedAt)}.log`);
  ...
  appendLine(logStream, "====================================");
  appendLine(logStream, "Stock Radar V2.3 AI 推薦回饋學習自動驗收測試");
  appendLine(logStream, "====================================");
  ...
  appendLine(logStream, `DB流程：${skipDb ? "略過" : "執行 setup / feedback / learning"}`);
  ...
  const tasks = [
    ...
    { title: "V2.3 setup script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "setupAiFeedbackLearningTables.js")], cwd: apiDir },
    { title: "V2.3 feedback script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "generateAiFeedbackLearning.js")], cwd: apiDir },
  ];

  if (!skipDb) {
    tasks.push({ title: "建立 V2.3 AI 推薦回饋學習資料表", command: "npm", args: ["run", "ai-feedback:setup"], cwd: apiDir });
    tasks.push({ title: "產生 V2.3 AI 推薦回饋學習", command: "npm", args: ["run", "ai-feedback:generate", ...(tradeDate ? ["--", tradeDate] : [])], cwd: apiDir });
  }

  tasks.push({ title: "V2.3 靜態 / API 驗收", command: "npm", args: ["run", "v23:check", ...(apiBaseUrl ? ["--", `--api=${apiBaseUrl}`] : [])], cwd: apiDir });
...
main().catch((error) => {
  console.error("V2.3 自動驗收測試失敗：", error);
  process.exit(1);
});
