...
async function main() {
  ...
  const logFile = path.join(logDir, `v15-acceptance-${formatDateTime(startedAt)}.log`);
  ...
  appendLine(logStream, "====================================");
  appendLine(logStream, "Stock Radar V1.5 自動驗收測試");
  appendLine(logStream, "====================================");
  ...
  if (!skipDb) {
    tasks.push({ title: "建立 V1.5 市場風險資料表", command: "npm", args: ["run", "market-risk:setup"], cwd: apiDir });
    if (!skipImport) tasks.push({ title: "匯入 TAIFEX 台指期市場風險", command: "npm", args: ["run", "market-risk:import"], cwd: apiDir, optional: true });
    tasks.push({ title: "產生夜盤修正分數", command: "npm", args: ["run", "market-risk:score", ...(tradeDate ? ["--", tradeDate] : [])], cwd: apiDir });
  }

  tasks.push({ title: "V1.5 靜態驗收", command: "npm", args: ["run", "v15:check", ...(apiBaseUrl ? ["--", `--api=${apiBaseUrl}`] : [])], cwd: apiDir });
...
main().catch((error) => {
  console.error("V1.5 自動驗收測試失敗：", error);
  process.exit(1);
});
