...
async function main() {
  ...
  const logPath = path.join(logDir, `v30-acceptance-${formatDateTime(startedAt)}.log`);
  ...
  appendLine(logStream, "====================================");
  appendLine(logStream, "Stock Radar V3.0 實戰交易輔助系統自動驗收測試");
  appendLine(logStream, "====================================");
  ...
  appendLine(logStream, skipDb ? "DB流程：略過 setup / trading assist generate" : "DB流程：執行 setup / trading assist generate");
  ...
  const tasks = [
    ...
    { title: "V3.0 setup script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "setupTradingAssistantTables.js")], cwd: apiDir },
    { title: "V3.0 generate script 語法檢查", command: "node", args: ["--check", path.join(apiDir, "scripts", "generateTradingAssistantPlans.js")], cwd: apiDir },
  ];

  if (!skipDb) {
    tasks.push(
      { title: "建立 V3.0 實戰交易輔助資料表", command: "npm", args: ["run", "trading-assist:setup"], cwd: apiDir },
      { title: "產生 V3.0 實戰交易輔助建議", command: "npm", args: ["run", "trading-assist:generate"], cwd: apiDir },
    );
  }

  const checkArgs = ["run", "v30:check"];
  if (apiBaseUrl) checkArgs.push("--", `--api=${apiBaseUrl}`);
  tasks.push({ title: "V3.0 靜態 / API 驗收", command: "npm", args: checkArgs, cwd: apiDir });
...
main().catch((error) => {
  console.error("V3.0 自動驗收測試失敗：", error);
  process.exit(1);
});
